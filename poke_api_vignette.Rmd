...
The output of the `stats` function is a tibble with the Pokemons name, Height in meters, and Weight in kilograms.
...
Here we test the `dex` function. We are using regions with dex ids 1 through 5, and only returning results on our same four Pokemon from the previous two examples. Any `N/A` means that the Pokemon is not in that Pokedex.
